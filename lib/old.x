...
class LibSecp256k1FFI {
  ...
  /// hashing function (e.g., SHA-256 from `package:crypto`).
  ///
  /// WARNING: This signing method does NOT use a prefix. 
  ///
  /// Returns the DER-encoded ECDSA signature as a [Uint8List], or `null` if signing fails.
  static Uint8List? signDigestInsecure(
    Uint8List digest,
    Uint8List privateKeyBytes
  ) {
    if (!_isInitialized || _ecdsaSignDart == null || _ecdsaSignatureSerializeDerDart == null || _context == null) {
    ...
    try {
      ...
      final ffi.Pointer<ffi.Uint8> msg32Ptr = arena<ffi.Uint8>(32);
      msg32Ptr.asTypedList(32).setAll(0, digest);
  ...
  // static BigInt? verifyDigestInsecure({
  ...
  //   final Uint8List pubBytes = _ensureUint8List(publicKeyBytes);
  //   final Uint8List sig = _ensureUint8List(signature);
  //   final Uint8List msg = _ensureUint8List(digest);

  //   if (pubBytes.length != 33) {
  //     throw ArgumentError(
  //       "Public key must be 32 bytes for secp256k1. Got ${pubBytes.length}",
  //     );
  //   }
  ...
  //     // Allocate native memory and copy data to it.
  //     // seckeyPtr will be modified in place by the C function.
  //     final ffi.Pointer<ffi.Uint8> publicKeyPtr = arena.allocate<ffi.Uint8>(
  //       pubBytes.length,
  //     );
  //     final ffi.Pointer<Secp256k1EcdsaSignatureStruct> sigStructPtr =
  //         arena<ffi.Uint8>(64).cast<Secp256k1EcdsaSignatureStruct>();
  //     final ffi.Pointer<ffi.Uint8> sigPtr = arena.allocate<ffi.Uint8>(
  //       sig.length,
  //     );
  //     final ffi.Pointer<ffi.Uint8> msgPtr = arena.allocate<ffi.Uint8>(
  //       msg.length,
  //     );

  //     // Get a Uint8List view into the allocated native memory to copy data easily
  //     publicKeyPtr.asTypedList(pubBytes.length).setAll(0, pubBytes);
  //     sigPtr.asTypedList(sig.length).setAll(0, sig);
  //     msgPtr.asTypedList(msg.length).setAll(0, msg);
      

  //     // Call the native function
  //     // 'context' here is the ffi.Pointer<Secp256k1Context> you created with secp256k1_context_create
  //     final int success = _ecdsaVerifyDart!(_context!, seckeyPtr, tweakPtr);
  ...
  static BigInt? _pureDart_addScalar(
    ...
    final newScalarBig = (ilInt + privKeyInt) % N;
